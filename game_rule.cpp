...
void DrawGameRules(int x, int y, int lineHeight)
{
	// 设置文本样式（游戏规则专用）
	setbkmode(TRANSPARENT);
	settextstyle(30, 0, _T("微软雅黑")); // 字体大小适中，便于阅读
	settextcolor(BLACK);

	// 按行拆分游戏规则（每行长度适配界面，避免超出显示范围）
	const TCHAR* rules[] = {
 _T("玩法说明：在空白地图上，通过布置炮塔构建行进路线，引导怪兽按阵型往复移动；可通过搭建或出售出口处的炮塔，"),
 _T("调整怪兽的移动路径，尽可能延长其移动距离，让炮塔持续轰击怪兽以达成更高输出。"),
 _T(""), // 空行分隔
 _T("高阶玩法提示：选择武器部署至地图后，怪物将向基地发起进攻。游戏提供具备攻击能力的大炮"),
 _T("防御工事的路障。核心策略为合理布置大炮，并配合路障墙限制怪物行进路线，最大化攻击效率。"),
 _T("所有武器可自动攻击射程范围内的怪物，且可随时对已部署武器进行升级；金币用于购买和升级武器，消灭怪物可获取金币奖励。"),
 _T("若怪物抵达终点，玩家生命值将扣除；拆除任意大炮可返还50%购置金币。"),
 _T(""), // 空行分隔
